...
* Reduction and the AST
  ...
  Take for example, the humble =λ=. Copying from above, we have:
   ...
   Let =μ= be bound to the primitive =#M[μ]= then we have

   =~(μ params body)= => =#(#M[μ] params body)=
   ...
   makes them very brittle. As of yet I need very few of them. They're all
   related to creating and manipulating structures which are outside of the
   scope of the syntax..
...
** Summary
   ...
   1) ~(X . Y) <=> #(~X . Y)
   2) #(#M[f] . tail) => always escapes to implementation
   3) #(#F[f] . tail) => invokes foreign code iff tail is fully reduced.
