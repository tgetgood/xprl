...
* Channels and Streams
  ...
  the syntactic return position. The return stream can be manipulated as a
  stream within a ν construct. But a ν returns nothing, rather it connects
  various components into a dataflow network. It's possible that a ν could pass
  a return stream to a channel and thus expose it as a value in some part of a
  program, but that part of the program wouldn't see different from any other
  stream.
...
* The Continuation Map and Emission
  ...
  language. Rather it is maintained by the runtime and can be modified within
  νs. I don't want to write code in cps if I can avoid it.
...
* ν
  ...
  A ν takes a continuation map and builds a computation graph. It does not
  return anything syntactically, but it does have a :return channel in the ccs
  passed to it and its subcomputations are free to send messages to it.
  ...
  finished when all subcomputations are finished. I haven't yet decided upon
  whether it should be possible to wait for one ν to "finish" before doing
  something. I'd rather not since that would add considerable complexity to the
  language with minimal gain.

  A much better way to deal with sequencing is via channels which close. Step
  one is "complete" when the corresponding channel can receive no more input,
  etc..
  ...
  =connect=, as the name implies, connects a form to a continuation map. This is
  the exact same construct by which an Emission which reaches the runtime is
  acted upon.
  ...
  Note: I'd rather not allow waiting on a conc since a lot of ill can come of it
  and I'm not sure the good outweighs that. I need to write more code to see if
  it's actually necessary. I'm not even convinced you ever need to nest seq &
  conc.
  ...
  channel connecting f to g provides a data dependency. g will park on reading
  ...
  Since a ν receives a syntactic return stream, ((comp (map f) (map g)) xs) is
  equivalent to ((map g) ((map f) xs)), as it should be.

  This is a little ugly though because f and g must both be νs for this version
  of comp to make sense. comp of μs is a different operator. Some types are
  inherent in the calculus as such.
        ...
        (ν ccs
           ...
           (let [state (last-chan init)]
             (seq
              ...
              ;; state stream.
              ;; This works because values, when connected, emit themselves on
              ;; :return
              (connect (first state) ccs))))))

    (defn into [coll]
      (accumulate conj coll))

    ((into []) ((map inc) ((filter even?) [1 2 3 4 5])))
                                            ; ⇒ [2 4 6]
  #+END_SRC
