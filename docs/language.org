...
** Pairs vs Cons Cells
   ...
   Notably this means that we don't need to assume that functions take lists of
   parameters. Languages that assume functions to take a list of paramters must
   resort to a kludge like =(fn [& args] ...)= if they want to support binding
   all passed values to a single name, whereas in xprl we can just write
   =(fn args ...)= and args will be bound to whatever the tail of the invoking
   expression is.

   Note that this is true even if the tail is not a list at all:
   =(f . {:x 3 :y 5})= and =(f . 3.14)= are perfectly well defined and don't box
   or coerce their tails into lists.

   Coercion could be implemented, but I have, as yet, no use for it.
...
** The μ Operator
   ...
   #+BEGIN_SRC clojure
     ...
     ;; respectively, but we're still talking about adding symbols, rather than
     ;; their referrents, which would be an error were we to try and do it.
   #+END_SRC
...
** Immediate Evaluation
   ...
   In reality it means "evaluate the following expression at some point after
   you have all of the information required to do so", which will remain
   ambiguous until we get into the details of the AST.
...
** An Exception?
   ...
   automatically evaluated. The same holds for the code reader =loadfile=.

   This isn't really an exception since =def= and =loadfile= are μs and are free
   to evaluate their arguments when and as they see fit.

   The low level language where you need to track evaluation by hand is very
   powerful, but hard to use. Higher level constructs attempt to provide an
   intuitive functional programming style so that you don't have to worry about
   the highlights and hazards of metaprogramming when you aren't
   metaprogramming.
* Reduction and the AST
  ...
  Take for example, the humble =λ=. Copying from above, we have:
   ...
   Which applies [x y] to whatever the symbol =f= is bound to. The μ to which f
   is bound might in turn choose to evaluate (lookup) x and y to do something
   with them.
   ...
   Let =μ= be bound to the primitive =#F[μ]= — =#F= denotes an "external
   function" even though they aren't generally functions. I'm leaning towards
   just "extern" as a less inaccurate name.
   ...
   FIXME: The rest of this is very dated.
...
* The Environment
  ...
  difficult to get right. So judicious use is still required.
* Pipes, Channels, and Streams
  ...
  A stream is an immutable, lazy, potentially unbounded, sequential data
  structure. A stream is a value, but it might not yet be known. Reading the
  first element off of a stream will return it if it is known right now, or it
  will park and wait for a value if it isn't. Calling =rest= on a stream returns
  the stream consisting of all but the first element and never parks.
...
* The Continuation Map and Emission
  ...
  messages can be logged, replayed, filtered, or even intercepted to sandbox a
  component. That makes a big difference.
...
* Ordering
  - Note taken on [2026-07-02 Thu 14:05] \\
    FIXME: This whole section is defunct.
...
* Stateful computation
  ...
  =map= takes elements of each of its input streams one at a time, so by priming
  =state= with an initial value, we can ensure that it's always one step ahead
  of the input stream =xs=.
...
** Work stealing
   - Note taken on [2026-07-03 Fri 10:16] \\
     The system this addresses is currently in flux and so this is not likely to
     reflect reality.
...
** Receivers                                                     :receivers:
   - Note taken on [2026-07-03 Fri 10:15] \\
     I no longer make a distinction between μ and ν, but everything said here about
     receivers still applies to μs.
...
** Namespaces and Dependencies
   ...
   lack of a better term. It could be a μ, it could be literal data. I don't
   think there's much else it could be. Of course even a function is sent as
   literal data + context of evaluation (also literal data) + possible analysis
   and compile cache + possible cache of ready to execute binary for a
   given arch.
