...
* The Environment
  ...
  in which a sexp is to be interpreted is attached to the sexp itself.

  When a μ is created, we create a new node called a Declaration which declares
  the parameters of the μ (thus wiping out previous bindings if we're shadowing
  something in the surrounding environment).

  When arguments are passed to a μ we add a new node called a Binding which
  binds a value to a symbol.

  We then add rules to the interpreter of the form so that when it encounters
  something like I-B-D-S, it checks that binding actually binds the declared
  variable and if it does replaces the expression by the bound value. I-B-S
  triggers an undeclared variable error, etc.. The complete set of rules is
  extensive and still in flux. To be frank it's too complicated and I think it
  needs reconceptualisation, but not just yet.

  Finally, when a form is interpreted using read or loadfile, the context of
  evaluation must be incorporated into it. This is done adding a Context node
  to the root of the form which binds the namespace in which a form is defined
  to the form itself.
  ...
  Reference is static, even if it needed be looked up until runtime.
  ...
  difficult to get right. So avoid it unless necessary.
...
* Stateful computation
  ...
  The easiest way would be to have a μ which from tail position sends messages
  ...
  over the order of delivery from disparate senders, so how do we know that your
  future self will receive its new state before it receives its next input?

  Sadly you can't. But you can combine the state stream with the input stream
  creating a stream of paired values — the =seq= in =map= ensures that the
  =state= stream is updated before it is read each round.

  #+BEGIN_SRC clojure
    (defn accumulate [rf init xs]
      ...
      (let [[st ch] (last-chan init)]
        (seq
         ;; Using a state channel to pass output back to yourself is a
         ;; common pattern.
         ;; Here we reduce into the state channel without returning
         ;; anything.
         (with-channels {:state st}
           ((map (fn [[state x]] (emit :state (rf state x))))
            ;; N.B.: the "combination" stream accesses its substreams on demand,
            ;; so we only ever request the next state strictly after the
            ;; previous step has finished emission.
            (combine state xs)))
    ...
    (defn into [coll xs]
      (accumulate conj coll xs))

    (into [] (map inc (filter even? [1 2 3 4 5]))) ; ⇒ [2 4 6]
  #+END_SRC

  Why am I not just calling this =reduce=?
  ...
  the language guarantees. We call into above to get a vector fully realised in
  memory. By themselves, map, filter, etc. return ~streams~ which may not be
  realised (i.e. may park when you try to read values) and which are not
  intended as random access datastructures.

  Note that in this case, =state= could be a normal pipe and =combine= would act
  as coordination between writers and readers by always calling the next
  element. Howeve, this pattern breaks down in situations where you don't want
  to update the state at each step. Are such situations still pertinent? I can't
  think of any.
