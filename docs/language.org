...
* The Environment
  ...
  can think of a μ defined in a namespace as being passed a namespace as well as
  its arguments. The version of the namespace passed is fixed statically, so we
  always know exactly what we're calling. But having the expected namespace
