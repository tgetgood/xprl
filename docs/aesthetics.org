...
* How to Read this Document
  Many of these sections overlap partially and often seem to contradict each
  other. They probably do contradict each other sometimes.

  This document is an ongoing conversation with myself. Each thread brings
  different ideas to the fore and glosses over others. If a thread is still
  here, that means I still think it has something important to say, even if it
  does contradict several other things I've said.
...
** Perspective: Interpretation all the way down
   ...
   These are different takes on the same theme from different points in the
   past. [[*Perspective: Linguistic Archaeology][Perspective: Linguistic Archaeology]] is another one. They each have
   their points and they are none of them complete.
*** Take 1
    - Note adapted from [2020-01-06 Mon] on paper
    ...
    So Barthes put it too harshly: the author isn't dead, they're just another
    reader (of their own message).
*** Theory
    Data is the medium in which computation can occur.

    But I can't define data and don't want to beg the question, so I'll try to
    avoid that term.
    ...
    For now, let's just consider literal data as a subset of all possible
    messages.

    The subset of possible messages depends on the perceptive capabilities of
    the interpreter. In the case of a modern digital machine it is the set of
    all possible binary blobs of finite length. In the case of living organisms
    it is much richer. Possibly only on the surface.

    Such a subset can only exist with respect to a given interpreter. The
    interpreter in question will often be implicit in what follows.
    ...
    structure between the message and the interpreter which emits it unchanged.
    ...
    "directly interpreted". Whatever that means. In practice it might refer to
    the process by which binary words act directly and electrically upon the
    hardwired circuits of a digital computer. Of course that's not the only
    possibility.
...
** Perspective: Linguistic Archaeology
   ...
   *text* is inscribed on the tablet, but if no one knows how to read it,
   ...
   interpreters; see whitespace and polyglot code. Or a text might be a message
   to one receiver but not to another. Think of a luks device with no header.
...
** Perspective: A Generalisation of Homoiconicity
   ...
   These are precisely the structures one calls "literal data" in clojure.
   Quoted forms are notably *not* literal data because the quoting is lost and a
   second pass through the cycle will evaluate the form.
   ...
   interpreters but only data with respect to one of them.
...
** Perspective: Entropy (Cybernetics)
   ...
   There's a similarity between the negentropy of communication in this context
   and the negentropy by which Schrödinger defines life.

   Above I've defined communication such that it can only exist within a system
   of living agents, but could we invert that and posit that communication is
   one of the defining characteristics of life?

   These observations aren't new, but they're widely dismissed as irrelevant. I
   hope to convince you otherwise.
** Holons and Holarchy
   ...
   Each unit (The word "object" used to be devoid of ideological baggage — and
   I suspect that's why it was used — but that's no longer the case) receives
   input on channels and emits output to channels. The unit has names for these
   channels since they must be referred to, but no knowledge of what's on the
   other side.

   This gives a unit autonomy from within. Given a set of inputs, the unit will
   do its thing, and that thing cannot be overridden or perverted from the
   outside. But since the unit has no notion of where inputs come from or where
   outputs go, when viewed from above it is fully subordinate to those "higher"
   units which decide how to network the channels of "lower" units together.

   Notably this removes the need for a global name registry, or "phone book" by
   which to route messages through the system. units have references (by value)
   to other units, and connect them together via channels. The subordinate units
   know the names of the channels to which they may emit, but they have no
   direct access to, or even knowledge of, other units.

   Applying this notion all the way down to the language primitives is the
   overarching design problem of this language. Progress has been slow, but the
...
** Observationally Immutable                                  :immutability:
   ...
   better idea. Roc has picked this up as well.

   With strict immutability you can't have cycles, which means reference
   counting will suffice for GC, but if the current reference count is 1,
...
** Heterogeneous Compute and Distribution                     :distribution:
   ...
   have lots of fancy algorithms for optimising streaming workflows in the face
...
** Reference by Value                                              :context:
   ...
   functions =fact= and =+= and assume that these are the things you're
   referring to.
   ...
   aren't locked by your dev machine but can be chosen by a CI server (>=1.0
   anyone?).
...
** Message Passing as the Only Side Effect                        :emission:
   ...
   of literal data that eventually drives a speaker cone. If the sound produced
   ...
   compiler do it. That is a solved problem nowadays.
...
** Interpretation All the Way Down                               :receivers:
   ...
   Think of a lambda as defining a new interpreter. The new interpreter is
...
*** A Potential Implementation                        :context:idealhashtree:
    ...
    ideal hash tree whose keys are the hashes of the actual values we want to
    store. So maybe an associatively indexed hash set?
...
** Types
   ...
   an an integer and the other as a sequence of unicode characters? You
   ...
   These, frankly, don't interest me. Gödel showed that any such system must
   spend an eternity chasing its own tail and never get it. The recent hype
   around homotopy type theory largely stems from the fact that the bulk of the
   theory (theorems) are machine generated by a theorem prover. Which if you
   aren't a hopeless technogroupie should set off alarms. As an aside, I
   actually think topology is the direction we should go when trying to
   formalise programs, but the dependence on logic is historical and often just
   a yak shave.
   ...
   expressing whatever we want to express (are there only countably many
   inconsistencies?), thus solving incompleteness.
...
** The Stack
    ...
    machines when defining languages stem more or less directly from Dijkstra's
    ...
    so well that stack ops are part of the instruction set of modern chips, and
    ...
    The problem is obvious if you have ever worked with javascript pre ES6. It's
    also apparent in Rust's red/blue function kerfuffle when you realise the
    difference between red and blue is that one uses the return stack and the
    other uses a scheduler / event loop.
    ...
    cleverness of this approach. But a great kludge is still a kludge,

    Concretely; I hypothesise that if we rethink the stack abstraction we
...
** Functions, Proceedures, Coroutines, and Transducers  :emission:receivers:
   ...
   Orthogonal issues: to whom do we "return" these values? and when?
   ...
   Note [2025-10-04 Sat]: The following ignores the complexity of preserving the
   order of the outputs of a sequence operation with respect to their inputs,
   It's also very out of date towards the end.

   FIXME: Focus on motivation and theoretical concerns in this document and
   remove the speculation about possible implementations.
   ...
   return a collection, if returns multiple times.
...
** SI is Overrated
   - Note taken on [2025-10-02 Thu 18:11] \\
     The main argument is still one I stand by, but this piece sounds more juvenile
   ...
   replace one arbitrary quantity, roughly the length of a human step, with
   another arbitrary quantity: the distance from the north pole to the equator
   divided by ten million, which is... roughly the length of a human step.

   (Recently we've updated that to be the distance light travels in the length of
   a human step divided by the speed of light, which is independent of the
   geometry of the earth, but otherwise useless.)
   ...
   Curiously, while the width of thumbs and the lengths of legs vary
   considerably from one person to another, the ratio varies far less, so this
   exercise tends to work for most people. Of course the "roughly" is critical,
   this isn't for building airplanes.

   Now, given a map scaled 1cm:1km, you know that one arbitrary distance on the
   map is equal to one hundred thousand times that same arbitrary distance on
   the ground (exactly!). Good luck with that. Of course you could approximate
   with the width of your pinky and a thousand (big) steps, but what have you
   gained from that?
   ...
   them, but how is it useful to anyone if the sector size on my disk is
   4.096 KB?
...
** Editor
   ...
   slime/cider/rope, or any other deeply interactive editing experience. It's
   just C# tooling abstracted to work to some extent with all languages.

   I want a bona fide residential programming environment that merges the
   editor, the program, and the language itself into a contiguous whole.

   Of course we have to solve the smalltalk vm problem, by which I mean the blob
   of arcane binary which is inscrutable execpt to itself, making version
   control very difficult. Git just doesn't cut it. None of the current methods
   really do.

   What do you do if you are trying to modify self-insert-command and screw up?
   In emacs you can just kill the server and go back to the state in the config.
   What can you do if that config itself is part of the system that has been
   bricked? Probably nothing.

   I conjecture that tracking providence (context) all the way back from your
   current state to the version of the language you started with (plus an empty
   program), we can have a form of version control, or at the very least an
   undo-tree. I expect it will be a lot more complicated than that sentence lets
   on.
** Orchestration without Containerisation
   ...
   services running in a cluster be as light or lighter than unix processes?
...
** Browser
   The modern web browser is a monumental ball of mud that works shockingly
...
** OS
   ...
   windows into the same thing, running each other's programs as appropriate.
